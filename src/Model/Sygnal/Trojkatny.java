...
public class Trojkatny extends SignalGenerator {
	...
	public Signal generate() {
		...
		for (double i = startingTime; i < time + startingTime - 1 / (2 * frequency); i = i + revFreq) {
			...
			if( i >= (k * interval + startingTime) && i < (quantization * interval + k * interval + startingTime)) {
				result.y.add(amplitude * (i - k * interval - startingTime) / (quantization * interval));
			}
			else if( i >= (quantization * interval + startingTime + k * interval) && i < (interval + k * interval + startingTime)) {
				result.y.add((-amplitude * (i - k * interval - startingTime) / (interval * (1 - quantization))) + (amplitude / (1 - quantization)));
			}
			else {
				result.y.add(0.0);
			}
